...
public class RestaurantController {
    ...
    public List<Restaurant> list() {                                                            // Restaurant의 목록을 얻는 List() 메서드를 선언한다.
        ...
        List<Restaurant> restaurants = restaurantRepository.findAll();                                    // repository에 있는 Restaurant List를 모두 restaurants List에 저장한다.
    ...
    public Restaurant detail(@PathVariable("id") Long id) {                                     // @PathVariable로 id를 선언하고 전달받은 매개변수 id를 매칭시켜준다.
        ...
        //Restaurant restaurant = restaurantService.getRestaurantById(id);                                          // 기본정보 + 메뉴정보를 한번에 돌려준다, 하나의 가게 정보를 얻을 수 있다.

        Restaurant restaurant = restaurantRepository.findById(id);                                        // repository List에서 전달받은 id로 해당 정보를 찾고 그 객체를 restaurant에 저장한다.
